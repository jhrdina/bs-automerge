...
module Make = (ActorId: CommonAPI.Stringifiable) => {
  ...
  module Json = {
    ...
    module ConflictValues = {
      ...
      let fold = (f, t, acc) =>
        Array.fold_left(
          (acc, actorIdStr) =>
            switch (ActorId.ofString(actorIdStr)) {
            | Some(actorId) =>
              f(actorId, t->Js.Dict.unsafeGet(actorIdStr), acc)
            | None => acc
            },
          acc,
          t->Js.Dict.keys,
      ...
      let get = (actorId, t) => Js.Dict.get(t, actorId |> ActorId.toString);
    };
  ...
  let make = actorId => AMJsRe.initWithActorId(actorId |> ActorId.toString);
